...
class BeerGameEnv(Env):
    ...
    def __init__(self):
        ...
        if self.AI_Order_Plus != True:
            ...
            Relative_Min_Order = -200                        # Amount relative to the incoming the order the agent
    ...
    def reset(self):
        ...
        if self.Bootstrapped_Teams:
            ...
            Boot_Team_Theta = np.around(np.random.uniform(min(Boot_Theta_Range), max(Boot_Theta_Range), Draws) ,Rounding)
            Boot_Team_Alpha = np.around(np.random.uniform(min(Boot_Alpha_Range), max(Boot_Alpha_Range), Draws) ,Rounding)
...
if __name__ == '__main__':
    ...
    if mode == "Train":
        ...
        Full_Hist = dqn.fit(env, nb_steps=5e5, visualize=False, verbose=2)
